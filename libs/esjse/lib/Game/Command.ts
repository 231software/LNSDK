...
export class FMPCommand{
    ...
    static register(command:FMPCommand):boolean{
        ...
        const esCmdPermission={
            ...
            default: (()=>{switch(command.permission){
                case FMPInternalPermission.Any:
                case FMPInternalPermission.Admin:return Enums.PermissionDefault.True;
                case FMPInternalPermission.GameMasters:
                case FMPInternalPermission.HostPlayer:
                case FMPInternalPermission.Console:
                case FMPInternalPermission.Internal:return Enums.PermissionDefault.Operator
            }})()
        }
        FMPLogger.info("指令注册：添加参数和重载")
        const esCmdData={
            description,
            permissions:[permissionRoot+"."+command.name],
